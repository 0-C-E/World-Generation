...
fn handle_request(request: Request, url: &str, full_url: &str, state: &mut ServerState) {
    match url {
        ...
        "/viewer.js"       => { let js = inject_config(VIEWER_JS, state); respond(request, "application/javascript", js); }
        "/night-mode.js"   => { let js = inject_config(NIGHT_MODE_JS, state); respond(request, "application/javascript", js); }
        "/debug"           => { let html = DEBUG_HTML.replace("{{WORLD_FINGERPRINT}}", &state.world_fingerprint); respond(request, "text/html; charset=utf-8", html); }
        "/debug.css"       => respond(request, "text/css", DEBUG_CSS),
        "/debug.js"        => { let js = inject_config(DEBUG_JS, state); respond(request, "application/javascript", js); }
        ...
        "/islands.json"    => {
            state.ensure_islands_json();
            respond(request, "application/json", state.islands_json.as_deref().unwrap());
        }
...
fn handle_tile(request: Request, url: &str, state: &mut ServerState) {
    ...
    let _      = request.respond(Response::from_data(png.clone()).with_header(header).with_header(cache));
}
...
fn handle_debug_tile(request: Request, url: &str, state: &mut ServerState) {
    ...
    match render_debug_tile(&mut state.world, z, x, y) {
        Some(png) => {
            ...
            let _ = request.respond(Response::from_data(png).with_header(header).with_header(no_cache));
        }
        None => {
            let _ = request.respond(Response::from_string("Not Found").with_status_code(404));
        }
...
fn handle_cities_viewport(request: Request, full_url: &str, state: &mut ServerState) {
    ...
    let _ = request.respond(Response::from_string(json).with_header(header).with_header(no_cache));
}
...
fn handle_outline(request: Request, url: &str, state: &mut ServerState) {
    let id_str = url.trim_start_matches("/outline/").trim_end_matches(".json");
    ...
    let json = state.island_outlines.as_ref().unwrap()
        .get(&rid).map(String::as_str).unwrap_or("[]");
...
fn islands_to_json(islands: &[Island]) -> String {
    let entries: Vec<String> = islands.iter().map(|i| {
        format!(
            "[{},{},{},{},{},{},{},{},{},{}]",
            i.id, i.centroid.0, i.centroid.1, i.city_count,
            i.bounds.min_x, i.bounds.min_y, i.bounds.max_x, i.bounds.max_y,
            i.is_world_spawn as u8, i.spawn_order,
        )
    }).collect();
    format!("[{}]", entries.join(","))
}
    ...
    for island in &islands {
        ...
        for gy in 0..gh {
            for gx in 0..gw {
                ...
                let is_edge = gx == 0 || gx + 1 == gw || gy == 0 || gy + 1 == gh
                    || !grid[gy * gw + (gx - 1)]
                    || !grid[gy * gw + (gx + 1)]
                    || !grid[(gy - 1) * gw + gx]
                    || !grid[(gy + 1) * gw + gx];
